...
## 1. 조인
 - 조인조건에서는 넘어갈쪽의 컬럼에 인덱스가 걸리면 빠르다.
...
- sort단계 : 먼저 각 테이블 별로 해당 테이블 조건대로 추출(SGA에서 처리해줌) 
- merge단계 : 추출된 두 테이블을 연결하여 결과값 추출(PGA에서 각각의 서버프로세스에서 처리해야함)
...
```
- dept은 INDEX FULL SCAN (정렬시켜서 내보내기위해 index풀 스캔을 탄다. 정렬된 순서대로 테이블로 넘어간 후 RESEARCH인 값들을 넘겨준다 )
- emp은 조건이 없으므로 TABLE ACCESS FULL  
- 두 개를 merge처리 해준다
